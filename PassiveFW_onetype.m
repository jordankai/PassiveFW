...
function [SD,RD,traces,TT]=PassiveFW_onetype(dt,fmax,delka_t,source_number);
...
%**************************************************************************
% case 1: three low-velocity layers
% Cs   = [300 200 500];   % Shear wave velocity [m/s]
% Cp   =[600 1700 2000];
% Rho  = [1 1 1 ]*2000;    % Mass density of layers (kg/m^3)
% H    = [10 10 inf];      % depth of layers [m]
% Qs=[100 100 100];
% Qp=Qs;
% Damp_p = 1./(2*Qp); % damping (hard-coded here ...)
% Damp_s = 1./(2*Qs);

% case 2: normal two-layer model
% Cs   = [220 440];   % Shear wave velocity [m/s]
% Cp   =[380 760];
% Rho  = [1 1 ]*2000;    % Mass density of layers (kg/m^3)
% H    = [10 inf];      % depth of layers [m]
% Qs=[30 30];
% Qp=Qs;
% Damp_p =[0 0] ;%1./(2*Qp); % damping
% Damp_s = [0 0];%1./(2*Qs);

% case 3: half-space
Cs   = [440];   % Shear wave velocity [m/s]
Cp   =[760];
Rho  = [1 ]*2000;    % Mass density of layers (kg/m^3)
H    = [inf];      % depth of layers [m]
Qs=[50];
Qp=Qs;
Damp_p = 0; % damping
Damp_s = 0;
...
%**************************************************************************
%%%%%%%%%%%%  set point loads   %%%%%%%%%%%%%%%%%%%%%
Fx_flag=1; % point load in x-axis
Fy_flag=1; % point load in y-axis
Fz_flag=1; % point load in z-axis
if (Fx_flag==0) && (Fy_flag==0) && (Fz_flag==0)
    error('Non-zero value is assigned to any point load!')
end
Fx=Fx_flag*rand(1,source_number);
Fy=Fy_flag*rand(1,source_number);
Fz=-Fz_flag*rand(1,source_number); %% z-axis is positive in the upward direction

%fm=10*ones(1,source_number);  %% Wavelet Dominant Frequency
fm1=5;  %% min. Dominant Frequency
fm2=40;  %% max. Dominant Frequency
fm=rand(1,source_number)*(fm2-fm1)+fm1;  %% Random distribution of Wavelet Dominant Frequency
delay_t=4/fm1;             %% excitation time of delta function
%**************************************************************************
%%%%%%%%%%%%  set ambient source-receiver geometry    %%%%%%%%%%%%%%%%%%%%%
[sourceX,sourceY,receiverX,receiverY]=ambient_geometry_plot(source_number);
%[sourceX,sourceY,receiverX,receiverY]=S_R_geometry(source_number);

% do test for designed model,fixed parameters 
if source_number==1
    Fx=0.2*ones(1,source_number);
    Fy=0.5*ones(1,source_number);
    Fz=-1*ones(1,source_number); %% z-axis is positive in the upward direction
    sourceX=-110;
    sourceY=0;
    srt22=1;
    fm=20;
...
if logical(Fz_flag)
    [Ky_v,uk1_v,uk2_v,uk3_v]=Green_HV_delta_one(Cs,Cp,Rho,H,Damp_s,Damp_p,N,dt,y_max,delay_t,fmax,'v');
else
    Ky_v=0;uk1_v=0;uk2_v=0;uk3_v=0;
end
if logical(Fx_flag) || logical(Fy_flag)
    [Ky_h,uk1_h,uk2_h,uk3_h]=Green_HV_delta_one(Cs,Cp,Rho,H,Damp_s,Damp_p,N,dt,y_max,delay_t,fmax,'h');
else
    Ky_h=0;uk1_h=0;uk2_h=0;uk3_h=0;
end
...
ppm = ParforProgressbar(numIterations,'parpool', {'local', cpu_N},'progressBarUpdatePeriod',60,'title','Do calculation over multi-sources...');
...
parfor ii=1:source_number
    ...
    if logical(Fz_flag)  %% z-axis point load
        [Wavef_v,tt]=Green_HV_delta_two(N,dt,Ky_v,uk1_v,uk2_v,uk3_v,y_pos,fmax,'v');
        % Vertical component
        ut_temp=conv_wavelet(tt,Wavef_v(:,:,3),fm(1,ii)); %% calculation of all receivers due to each point source
        ut3_v=ut_temp(N_shift+1:end-N2,:);
        ut3_v=mute_kai(ut3_v,1.5*max(Cp),y_pos,dt); %% mute events faster than 1.5*max(Cp)
        
        % Radial compenent
        ut_temp=conv_wavelet(tt,Wavef_v(:,:,1),fm(1,ii)); %% calculation of all receivers due to each point source
    ...
    if logical(Fx_flag) || logical(Fy_flag) %% xy-axis point load
        [Wavef_h,tt]=Green_HV_delta_two(N,dt,Ky_h,uk1_h,uk2_h,uk3_h,y_pos,fmax,'h');
        % Vertical component
        ut_temp=conv_wavelet(tt,Wavef_h(:,:,3),fm(1,ii)); %% calculation of all receivers due to each point source
        ut3_h=ut_temp(N_shift+1:end-N2,:);
        ut3_h=mute_kai(ut3_h,1.5*max(Cp),y_pos,dt); %% mute events faster than 1.5*max(Cp)
        
        % Radial compenent
        ut_temp=conv_wavelet(tt,Wavef_h(:,:,1),fm(1,ii)); %% calculation of all receivers due to each point source
        ut1_h=ut_temp(N_shift+1:end-N2,:);
        ut1_h=mute_kai(ut1_h,1.5*max(Cp),y_pos,dt); %% mute events faster than 1.5*max(Cp)
        
        % Tangential compenent
        ut_temp=conv_wavelet(tt,Wavef_h(:,:,2),fm(1,ii)); %% calculation of all receivers due to each point source
