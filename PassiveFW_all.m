...
function [traces,TT]=PassiveFW_all(dt,fmax,fm1,fm2,delka_t,source_number,S_R_Geo,model);
...
Cs=model.Cs;     % Shear wave velocity [m/s]
Cp=model.Cp;  
Rho=model.Rho;    % Mass density of layers (kg/m^3)
H=model.H;      % depth of layers [m]
Qs=model.Qs;
Qp=model.Qp;
...
UK=Green_HV_delta_one_all(Cs,Cp,Rho,H,Damp_s,Damp_p,N,dt,y_max,delay_t,fmax);
...
parfor ii=1:source_number
    ...
    [Wavef,tt]=Green_HV_delta_two_all(N,dt,UK,y_pos,fmax);
    %*****************************************************************************
    % z-axis point load
    % Vertical component
