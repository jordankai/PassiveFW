...
function [Wavef,t]=Green_HV_Heaviside_diff(Cs,Cp,Rho,H,Damp_s,Damp_p,Ntps,dt,y_pos,delay_t,flag);
...
source_tt=heaviside(t).*exp(-delta_w*t); %%  Exponential Window Method
arrf=(fft(source_tt));
...
parfor ii=2:(length(w)-1) %% loops over the frequency
    ...
    qR = zeros(nL2,1);  qR(1)=arrf(ii)/2/pi;        % load vector for Radial
    qT = zeros(nL,1);   qT(1)=arrf(ii)/2/pi;        % load vector for Tangential
    qV = zeros(nL2,1);  qV(2)=arrf(ii)/2/pi;        % load vector for Vertical
...
for ii=1:3
    ...
    ut=ut.';
    ss3=zeros(size(ut));
    ss3(2:Ntps-1,:)=(ut(3:Ntps,:)-ut(1:Ntps-2,:))/2;  %% get the time derivation of heaviside response, that is detla response
    ss3(1,:)=0;
    ss3(Ntps,:)=0;
    Wavef(:,:,ii)=ss3;
end
