...
function [traces,TT]=PassiveFW_onetype_savefile(dt,fmax,delka_t,source_number,S_R_Geo);

% written by Zhang Kai especially for passive surface wave modelling
% IGGE 2021/7/28
% Parallel calculations over sources
% This version saves more memory through saving data into mat files,especially for dozens of cpu cores.
%**************************************************************************
...
ambient_geometry_plot(S_R_Geo); %% plot
...
t=0:dt:tmax;
N=length(t);
N2=0; %% mute the tail of the record
...
if logical(Fz_flag)
    Ky_v=Green_HV_delta_one_savefile(Cs,Cp,Rho,H,Damp_s,Damp_p,N,dt,y_max,delay_t,fmax,'v','D:\Modeling noise_v\');
else
    Ky_v=0;
end
if logical(Fx_flag) || logical(Fy_flag)
    Ky_h=Green_HV_delta_one_savefile(Cs,Cp,Rho,H,Damp_s,Damp_p,N,dt,y_max,delay_t,fmax,'h','D:\Modeling noise_h\');
else
    Ky_h=0;
end
...
parfor ii=1:source_number
    ...
    if logical(Fz_flag)  %% z-axis point load
        [Wavef_v,tt]=Green_HV_delta_two_savefile(N,dt,Ky_v,y_pos,fmax,'v','D:\Modeling noise_v\');
        % Vertical component
    ...
    if logical(Fx_flag) || logical(Fy_flag) %% xy-axis point load
        [Wavef_h,tt]=Green_HV_delta_two_savefile(N,dt,Ky_h,y_pos,fmax,'h','D:\Modeling noise_h\');
        % Vertical component
